...
private:
	ip::tcp::socket sock_;//ip - маршрутизируемый протокол сетевого уровня стека TCP/IP; tcp - протокол управления передачей
	enum { max_msg = 1024 };
...
void handle_accept(talk_to_client::ptr client, const boost::system::error_code & err)//Функция обрабатывания доступа
{
	client->start();
