...
#include <stdexcept>
typedef boost::asio::ip::tcp btcp;//используем для создания более короткого имени для типов
typedef boost::format bfmt;// boost::format позволяет безопасно форматировать строки
...
#define MEM_FN1(x,y)    boost::bind(&self_type::x, shared_from_this(),y) // Возвращает объект функции, вызов функции которого вызывает указатель на функцию члена 
#define MEM_FN2(x,y,z)  boost::bind(&self_type::x, shared_from_this(),y,z)

class talk_to_svr : public boost::enable_shared_from_this<talk_to_svr> // класс "связи с сервером";enable_shared_from_this позволяет который управляется умным указателем,безопасно создать доп экземпляры с типом shared_ptr
	, boost::noncopyable
{
...
public:
	...
	{
		if (!started_) return;
		started_ = false; //Функция останавливает работу клиента
		sock_.close();
	}
...
private:
	...
	{
		ofs.close();//Проверка на закрытость потокового ввода
		stop();
	}
	...
	void handlereaduntil(boost::system::error_code const& errcode, size_t bytes_transferred)
	{
		...
		try
		{
			...
			if (!boost::regex_search(headers, res, regFileSize)) // поиск среди записанного из потока размера файла и запись результата в res; regex_search - поиск подстроки в исходной строке, по заданному регулярному выражению
				throw std::runtime_error("[-] Ошибка определения размера файла.\n");
			filesize_ = boost::lexical_cast<uintmax_t>(res[1]);// lexical_cast - преобразование в тип uintmax_t
...
int main()
{
	...
	boost::this_thread::sleep(boost::posix_time::millisec(100));//posix_time позволяет не привязывать время к дате; sleep - усыпляет поток
